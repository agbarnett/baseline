...
get_pmcid_table = function(pmcid){
  ...
  if(nrow(percent)>0){
    g1 = filter(percent, column == 1) %>% # first group
      ...
      select('row','n1','N1')
    #
    g2 = filter(percent, column == 2) %>% # second group
      rename('N2' = 'sample_size',
             'n2' = 'stat1') %>%
      select('row','n2','N2')
    # could add third group ..
  ...
  # put into alternative format (update Jan 2025); only two groups for now
  # keep row number for ordering
  if(!is.null(percents)){
    percents1 = select(percents, v1, row, N1, n1) %>%
      rename('stat1' = 'n1',
             'sample_size' = 'N1') %>%
      mutate(stat2 = 100*stat1/sample_size, # make percent
             statistic = 'percent',
             column = 1)
    percents2 = select(percents, v1, row, N2, n2) %>%
      rename('stat1' = 'n2',
             'sample_size' = 'N2') %>%
      mutate(stat2 = 100*stat1/sample_size, # make percent
             statistic = 'percent',
             column = 2)
    percents = bind_rows(percents1, percents2)
  }
  if(!is.null(continuous)){
    continuous1 = select(continuous, v1, row, m1, sd1, n1) %>%
      rename('stat1' = 'm1',
             'stat2' = 'sd1',
             'sample_size' = 'n1') %>%
      mutate(statistic = 'continuous',
             column = 1)
    #
    continuous2 = select(continuous, v1, row, m2, sd2, n2) %>%
      rename('stat1' = 'm2',
             'stat2' = 'sd2',
             'sample_size' = 'n2') %>%
      mutate(statistic = 'continuous',
             column = 2)
    continuous = bind_rows(continuous1, continuous2)
  }
  long = bind_rows(percents, continuous) %>%
    mutate(pmcid = pmcid)
